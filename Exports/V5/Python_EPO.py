...
ax.scatter(
    ...
    z_scores_pixels[outliers_pixels[0], 2],
    color="red",
)
...
ax.scatter(
    z_scores_mean[outliers_mean[0], 0],
    z_scores_mean[outliers_mean[0], 1],
    z_scores_mean[outliers_mean[0], 2],
    color="red",
)
...
EfficientNet_M = EfficientNetV2S(
    input_shape=(img_res[0], img_res[1], img_res[2]), num_classes=2, dropout=0.5
)
# define new model
model = Model(inputs=EfficientNet_M.inputs, outputs=EfficientNet_M.outputs)
...
Debug_OUTPUT_DPS_freq = (
    32  # Debug_OUTPUT_DPS_freq: Debug image output frequency(epoch).
)
...
try:
    for epoch in range(1, max_epoch):
        ...
        if load_SUB_BRW and load_weights:
            ...
            #             ['yellow']) # OLD
            print_Color_V2(
                f"<light_red>Not loading weights<green>[<light_blue>BSR:<yellow>acc{{{95.675647:.4f}}}, <yellow>loss{{{0.0111:.4f}}}<light_magenta>|<light_blue>BTR:<green>acc{{{97.56456:.4f}}}, loss{{{0.002:.4f}}}]"
            )
        ...
        if acc > best_acc:
            print_Color_V2(
                f"<yellow>Improved model accuracy from <green>{best_acc} <yellow>to <green>{acc}<yellow>. <light_cyan>Saving model."
            )
            # Update the best_acc
            best_acc = acc
            if SAVE_FULLM:
            ...
            print_Color_V2(
                f"<light_red>Model accuracy did not improve from {best_acc}. Not saving model."
            )

        # If the loss is higher than the best_loss
        if loss < best_loss:
            print_Color_V2(
                f"<yellow>Improved model loss from <green>{best_loss} <yellow>to <green>{loss}<yellow>. <light_cyan>Saving model."
            )
            ...
            print_Color_V2(
                f"<light_red>Model loss did not improve from {best_loss}. Not saving model."
            )
        # Garbage Collection (memory)
        gc.collect()
        tf.keras.backend.clear_session()
