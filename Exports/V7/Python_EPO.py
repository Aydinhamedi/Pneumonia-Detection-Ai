...
try:
    for epoch in range(1, max_epoch):
        ...
        if acc > best_acc:
            print_Color_V2(
                f"<yellow>Improved model accuracy from<green>{best_acc:10f} <yellow>to <green>{acc:10f}<yellow>. <light_cyan>Saving model."
            )
            # Update the best_acc
            best_acc = acc
            if SAVE_FULLM:
        ...
        if loss < best_loss:
            print_Color_V2(
                f"<yellow>Improved model loss from <green>{best_loss:.10f} <yellow>to <green>{loss:.10f}<yellow>. <light_cyan>Saving model."
            )
