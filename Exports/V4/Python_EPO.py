...
ConvNeXtLarge_M = ConvNeXtLarge(include_top=True, input_shape=(img_res[0], img_res[1], img_res[2]), weights=None, classes=2, classifier_activation='softmax')
# define new model
...
EfficientNet_M = EfficientNetV2XL(input_shape=(img_res[0], img_res[1], img_res[2]), pretrained='imagenet21k-ft1k', num_classes=2, dropout=0.5)
# define new model
model = Model(inputs=EfficientNet_M.inputs, outputs=EfficientNet_M.outputs)
...
try:
    for epoch in range(1, max_epoch):
        ...
        print_Color(f'\n~*Epoch: ~*{epoch}~*/~*{max_epoch}~* | ~*[{STG}]', ['normal', 'cyan', 'normal', 'green', 'blue', 'green'], advanced_mode=True)
        # DP
        print_Color('Shuffling data...', ['yellow'])
        x_train, y_train = shuffle_data(x_train, y_train)
        ...
        SUB_history = model.fit(x_SUB_train,
                            ...
                            verbose='auto',
                            callbacks=[learning_rate_schedule_SUB,
                                        TerminateOnHighTemp_CB,
                                        early_stopping] if Use_ES_ONSUBT else [learning_rate_schedule_SUB,
                                                                                TerminateOnHighTemp_CB]
        )
